import type { Locale, ScoredPlace } from "@truebite/shared";
import { RealScoreBadge } from "../RealScoreBadge";
import { fmtReviews } from "@/lib/format";

/**
 * Rehber sayfası mekan satırı — TAM SUNUCU-RENDER (client JS / lazy highlights yok).
 * Konumsuz sayfa olduğu için mesafe yerine mahalle gösterilir. RealScoreBadge yeniden
 * kullanılır (sunucu bileşeni — "use client" yok).
 */
export function GuideSpotRow({
  place,
  rank,
  locale,
  district,
}: {
  place: ScoredPlace;
  rank: number;
  locale: Locale;
  district: string | null;
}) {
  const lead = rank === 1;
  return (
    <li
      className={`flex items-start gap-4 border-t py-5 sm:gap-6 ${
        lead ? "border-ink/20" : "border-line"
      }`}
    >
      <div
        className={`w-8 shrink-0 pt-1 font-mono text-base font-bold tabular-nums sm:w-12 sm:text-xl ${
          lead ? "text-ember" : "text-stone"
        }`}
      >
        {String(rank).padStart(2, "0")}
      </div>

      <div className="min-w-0 flex-1">
        <h3 className="truncate text-lg font-bold tracking-[-0.015em] sm:text-xl">{place.name}</h3>
        <div className="mt-1.5 flex flex-wrap items-center gap-x-2.5 gap-y-1 text-sm text-stone">
          {district && (
            <>
              <span>{district}</span>
              <span aria-hidden className="text-line">
                ·
              </span>
            </>
          )}
          <span>{fmtReviews(place.userRatingsTotal, locale)}</span>
        </div>
      </div>

      <RealScoreBadge place={place} lead={lead} locale={locale} />
    </li>
  );
}
